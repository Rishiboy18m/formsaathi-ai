...
export const FieldHighlight: React.FC<FieldHighlightProps> = ({
  ...
  const { boundingBox, labelBox, isLowConfidence, tamilName } = field;

  return (
    <>
      {/* 1. Neutral Label Indicator Box (where OCR read the label) */}
      {labelBox && (
        <div
          style={{
            left: `${labelBox.x}%`,
          ...
          className={`absolute rounded-sm border border-slate-400/50 bg-slate-300/10 pointer-events-none transition-all ${
            isSelected ? 'opacity-90 border-teal-500 bg-teal-200/20' : 'opacity-40'
          }`}
          title={`Label: ${tamilName}`}
        />
      )}

      {/* 2. Main Input Area Highlight Box (where the user writes!) */}
      <div
        onClick={(e) => {
          e.stopPropagation();
          onSelect(field);
        ...
        title={`${tamilName} (Write inside this area)`}
      >
        {/* Badge Tag */}
        <div
